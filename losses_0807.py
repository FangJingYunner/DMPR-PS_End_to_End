...
def compute_eval_results(data_batch, output, params):
    ...
    pred_points = get_predicted_points(output, params)
    eval_results = collect_error(data_batch, pred_points, params)
...
def calc_entry_dist(pd, gt):
    ...
    return pow((pd_x - gt_x) ** 2 + (pd_y - gt_y) ** 2, 0.5) # H/W = 3

def tmp_get_radian(pd, gt, mode="sepline"):
    if mode == 'sepline':
        pd_radian = math.atan2(pd.lenSepLine_y * 3,
                               pd.lenSepLine_x)
        gt_radian = math.atan2(gt.lenSepLine_y * 3,
                               gt.lenSepLine_x)
    else: # entryline
        pd_radian = math.atan2(pd.lenEntryLine_y * 3,
                               pd.lenEntryLine_x)
        gt_radian = math.atan2(gt.lenEntryLine_y * 3,
                               gt.lenEntryLine_x)
...
def collect_error(ground_truths_list, predictions_list):
    ...
    point_x_err = 0
    point_y_err = 0
    entryLine_coord_err = 0
    entryLine_radian_err = 0
    entryLine_cos_err = 0
    entryLine_sin_err = 0
    sepLine_radian_err = 0
    sepLine_cos_err = 0
    sepLine_sin_err = 0
    ...
    for i in range(num_samples):
        ...
        predictions = [
            ...
            if pred[0] >= config.CONFID_THRESH_FOR_POINT # 0.11676871
        ]
    
        for ground_truth in ground_truths:
            idx = match_gt_with_preds(ground_truth, predictions)
            if idx >= 0:
                predicted_confidence = predictions[idx][0]
                predicted = predictions[idx][1]
                # namedtuple::MarkingPoint::格式
                confidence_err += abs(predicted_confidence - 1.0)
                # available_err.append(abs(predicted.isOccupied - ground_truth.isOccupied))
                coord_dist_err += calc_coord_dist(predicted, ground_truth)
                point_x_err += abs(predicted.x - ground_truth.x)
                point_y_err += abs(predicted.y - ground_truth.y)
                entryLine_coord_err += calc_entry_dist(predicted, ground_truth)
                # EntryLine
                entryLine_radian_err += calc_radian(predicted, ground_truth, mode='entryline')
                entry_cos, entry_sin = calc_radian_cos_sin(predicted, ground_truth, 'entryline')
                entryLine_cos_err += entry_cos
                entryLine_sin_err += entry_sin
                # SepLine
                sepLine_radian_err += calc_radian(predicted, ground_truth, mode='sepline')
                sep_cos, sep_sin = calc_radian_cos_sin(predicted, ground_truth, 'sepline')
                sepLine_cos_err += sep_cos
                sepLine_sin_err += sep_sin
...
def match_gt_with_preds(ground_truth, predictions):
    """Match a ground truth with every predictions and return matched index."""
    max_confidence = 0.
    matched_idx = -1

    for i, pred in enumerate(predictions):
        effect_flag = match_marking_points(ground_truth, pred[1])
...
def get_confidence_list(ground_truths_list, predictions_list):
...
def calc_precision_recall(ground_truths_list, predictions_list):
    """Adjust threshold to get mutiple precision recall sample."""

    true_positive_list, false_positive_list \
        = get_confidence_list(ground_truths_list, predictions_list)
    ...
    FN = bisect.bisect_left(true_positive_list, config.CONFID_THRESH_FOR_POINT)
    TP = len(true_positive_list) - FN
    TN = bisect.bisect_left(false_positive_list, config.CONFID_THRESH_FOR_POINT)
