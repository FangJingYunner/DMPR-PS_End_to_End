...
class STDC2Net(nn.Module):
    def __init__(self, base=64, layers=[4, 5, 3], block_num=4, type="cat", num_classes=1000, dropout=0.20,
        ...
        self.x2 = nn.Sequential(self.features[:1])    #chanels = base // 2
        self.x4 = nn.Sequential(self.features[1:2])   #chanels = base
        self.x8 = nn.Sequential(self.features[2:6])   #chanels = base * 4
        self.x16 = nn.Sequential(self.features[6:11]) #chanels = base * 8
        self.x32 = nn.Sequential(self.features[11:])  #chanels = base * 16

        self.feat_channels = [base // 2, base, base * 4, base * 8, base * 16] #different chanels
        if pretrain_model:
...
class STDC1Net(nn.Module):
    def __init__(self, base=64, layers=[2, 2, 2], block_num=4, type="cat", num_classes=1000, dropout=0.20,
        ...
        self.x2 = nn.Sequential(self.features[:1])    #chanels = base // 2
        self.x4 = nn.Sequential(self.features[1:2])   #chanels = base
        self.x8 = nn.Sequential(self.features[2:4])   #chanels = base * 4
        self.x16 = nn.Sequential(self.features[4:6])  #chanels = base * 8
        self.x32 = nn.Sequential(self.features[6:])   #chanels = base * 16
...
if __name__ == "__main__":
    model = STDC1Net(base=32, num_classes=1000, dropout=0.00, block_num=4)
    model.eval()
